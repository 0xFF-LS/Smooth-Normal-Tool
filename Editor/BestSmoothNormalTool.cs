...
public class BestSmoothNormalTool : EditorWindow
{
    ...
    /// 当前写入的目标类型
    /// </summary>
    private WriteTargetType inner_writeTarget = WriteTargetType.Tanget;
    ...
    private bool inner_showHelprFoldout = false;
    ...
    /// <summary>
    /// 是否保存到原来的路径（覆盖原来的 Mesh）
    /// </summary>
    private bool inner_isSaveToSamePath = false;
    private bool isSaveToSamePathLast = false;
    private string dirPath = "ACGCharacter/Mesh";
    #endregion

    #region 打开 Best Smooth Normal Tool

    [MenuItem("LaoshaTools/法线平滑小工具 →(二法转切线，顶点色，UV)NPR", priority = 101)]
    ...
    private void OnGUI()
    {
        ...
        {
            GUILayout.Box("法线平滑小工具（构建二套法线）", this.inner_windowStyle, GUILayout.Height(50), GUILayout.ExpandWidth(true));        
        }
        ...
        GUILayout.BeginVertical();
        {
            ...
            GUILayout.Space(15);
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("设置为源路径", GUILayout.Height(30)))
            {
                Mesh mesh = GetMeshes(meshFilters, skinMeshRenders);
                SwithPath(mesh);
            }
            if (GUILayout.Button("设置为默认路径", GUILayout.Height(30)))
            {
                SwithPath(null);
            }
            EditorGUILayout.EndHorizontal();
            GUILayout.Label("保存路径", this.inner_textStyle);
            this.dirPath = GUILayout.TextArea(this.dirPath, GUILayout.ExpandWidth(true));
            GUILayout.Space(8);
            ...
            this.inner_showHelprFoldout = EditorGUILayout.Foldout(this.inner_showHelprFoldout, "帮助信息");
            if (this.inner_showHelprFoldout)
            {
                GUILayout.BeginVertical(this.inner_windowStyle, GUILayout.Height(120), GUILayout.ExpandWidth(true));
                {
                    GUILayout.Label("原作者信息：", this.inner_textStyle);
                    GUILayout.BeginHorizontal();
                    {
                       
                        GUILayout.Label("", this.inner_gitHubStyle, GUILayout.Height(32), GUILayout.Width(32));
                        GUILayout.BeginVertical();
                        GUILayout.Label("原项目所在 GitHub", this.inner_textStyle);
                    ...
                    GUILayout.Space(4);
                    GUILayout.Label("二次开发信息：", this.inner_textStyle);
                    GUILayout.BeginHorizontal();
                    {
                        GUILayout.Label("", this.inner_qqStyle, GUILayout.Height(32), GUILayout.Width(32));
                        GUILayout.BeginVertical();
                        GUILayout.TextArea("https://github.com/0xFF-LS/Smooth-Normal-Tool.git", GUILayout.ExpandWidth(true));
                        GUILayout.Label("1,本项目写入目标为;Tanget；关闭映射[0,1]；自判是否使用权重", this.inner_textStyle);
                        GUILayout.Label("2,可以手动把MeshFilter或Skinned替换新的Mesh;", this.inner_textStyle);
                        GUILayout.Label("3,记得材质上启用二套法线（如：切线描边，UV2平滑等）", this.inner_textStyle);
                        GUILayout.EndVertical();
    ...
    private void SwithPath(Mesh mesh)
    {
        if (mesh != null)
        {
            // 获取原模型在项目中的相对路径（如 Assets/Models/Test.fbx）
            string assetPath = AssetDatabase.GetAssetPath(mesh);
  
            // 如果 mesh 是代码生成的（不在工程里），dirPath 会为空，可以加个保底
            if (string.IsNullOrEmpty(dirPath))
            {
                dirPath = "Assets";
            }
            // 获取其所属目录（如 Assets/Models）
    ...
    private Mesh GetMeshes(MeshFilter[] meshFilters, SkinnedMeshRenderer[] skinMeshRenders)
    {
        bool isStop = false;
        // MeshFilter
    ...
    private (bool, Mesh) ExportMeshesCore(Mesh mesh)
    {
        ...
        string path = EditorUtility.SaveFilePanel("Save Separate Mesh Asset", absoluteDirPath, $"{mesh.name}_{typeName}", "asset");
